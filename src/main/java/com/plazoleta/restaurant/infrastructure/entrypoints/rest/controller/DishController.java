...
import com.plazoleta.restaurant.infrastructure.entrypoints.rest.dto.response.DishInternalResponseDto;
...
public class DishController {
    ...
    @GetMapping("/internal/{dishId}")
    @Operation(
            summary = "Get internal dish by id",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Dish found"),
                    @ApiResponse(responseCode = "404", description = "Dish not found")
            }
    )
    public ResponseEntity<DishInternalResponseDto> getInternalDishById(@PathVariable final Long dishId) {
        return ResponseEntity.ok(restaurantRestMapper.toDto(restaurantHandler.getDishById(dishId)));
    }
}
